...
class YLGIFImage : UIImage {
    ...
    convenience init?(named name: String!) {
        ...
        guard let data = NSData.init(contentsOf: NSURL.fileURL(withPath: path))
            //            NSData(contentsOfURL:NSURL.fileURLWithPath(path))
            else { return nil }
        self.init(data: data as Data)
    }
    
    convenience override init?(contentsOfFile path: String) {
        let data = NSData.init(contentsOf: NSURL(string: path)! as URL)
        //            NSData(contentsOfURL: NSURL(string: path)! as URL)
        self.init(data: data! as Data)
    }
