...
- (void)clickOK
{
    ...
    {
        [self.delegate areaPickerView:self didSelectArea:@[self.selectedProvince, self.selectedCity, self.selectedDistrict]];
    }
}
          ...
          forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    myView = [[UILabel alloc] initWithFrame:CGRectMake(0.0, 0.0, self.frame.size.width/3.0, 30)];
    if (component == 0)
    {
        myView.text = [self.provinces objectAtIndex:row];
//        return [self.provinces objectAtIndex:row];
    }
    else if (component == 1)
    {
        NSArray *citys = [self.citysForProvince objectForKey:self.selectedProvince];
//        return [citys objectAtIndex:row];
        myView.text = [citys objectAtIndex:row];
    }
    else if (component == 2)
    {
        NSArray *districts = [self.districtsForCity objectForKey:self.selectedCity];
//        return [districts objectAtIndex:row];
        myView.text = [districts objectAtIndex:row];
    }
//    return @"";
