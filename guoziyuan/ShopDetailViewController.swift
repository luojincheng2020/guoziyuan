...
class ShopDetailViewController: BaseSubViewController,UITableViewDelegate,UITableViewDataSource,FSSegmentTitleViewDelegate,FSPageContentViewDelegate{
    var login:UIButton?
    var img:YLImageView?
...
//        navigationController?.navigationBar.subviews.first?.alpha = 0.0
    ...
    func addNavRightBtn(_ title: String) -> UIBarButtonItem {
        ...
        btnRigth?.addTarget(self, action: #selector(ShopDetailViewController.back), for: UIControlEvents.touchUpInside)
...
//        navigationItem.rightBarButtonItem = addNavBtn(cityNow!)
        ...
        //
        //        if (tableView == nil) {
        tableView = BaseTableView(frame: CGRect(x: 0, y: -20, width: view.bounds.width, height: SCREENHEIGHT+20), style: .plain)
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        ...
        let labelTitle = UILabel(frame:CGRect(x:10,y:5,width:SCREENWIDTH-20,height:30))
        labelTitle.text = "该店在售商品"
        labelTitle.textColor = blackColor
        labelTitle.font = fontSize(size: 15)
        labelTitle.sizeToFit()
...
//        backview.addSubview(labelTitle)
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let labelTitle = UILabel(frame:CGRect(x:10,y:20,width:SCREENWIDTH-20,height:40))
        labelTitle.text = "该店在售商品"
        labelTitle.textColor = blackColor
        labelTitle.font = fontSize(size: 15)
        labelTitle.sizeToFit()
        let backview = UIView(frame:CGRect(x:0,y:0,width:SCREENWIDTH,height:100))
        backview.addSubview(labelTitle)
        backview.backgroundColor = lightColor
        
        titleView = FSSegmentTitleView(frame: CGRect(x: 0, y:50, width: view.bounds.width, height: 50), titles: ["葡萄", "猕猴桃", "苹果", "香蕉", "西瓜", "芒果","梨","哈密瓜"], delegate: self, indicatorType: FSIndicatorTypeEqualTitle)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if contentCell == nil {
                ...
                let titleArr:NSArray = ["葡萄", "猕猴桃", "苹果", "香蕉", "西瓜", "芒果","梨","哈密瓜"]
                let contentVCs:NSMutableArray = NSMutableArray()
                for title:String in (titleArr as? [String])! {
                    let fs:ShopScrollContentViewController = ShopScrollContentViewController()
                    fs.title = title
                    contentVCs.add(fs)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let bottomCellOffset:CGFloat =  (tableView?.rect(forSection: 1).origin.y)! - 40*2
//        print("4444=====\(scrollView.contentOffset.y)")
