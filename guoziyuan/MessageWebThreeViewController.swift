...
import MJRefresh
class MessageWebThreeViewController: BaseSubViewController ,WKScriptMessageHandler, WKUIDelegate {
...
//            })
    ...
    func userContentController(_ userContentController:WKUserContentController,
        ...
        if(sentData["method"] == "clickLine"){
            //获取商品名称
            let itemName = sentData["name"]!
            let alertController = UIAlertController(title: "点击的数据",
                                                    message: "\(itemName)",
            ...
            let okAction = UIAlertAction(title: "确定", style: .default, handler: {
                ...
                print("点击了确定")
                //调用页面里加入购物车js方法
//                self.webView!.evaluateJavaScript("addToCar('\(itemName)')",
...
//            self.present(alertController, animated: true, completion: nil)
        ...
        //返回
        if(sentData["method"] == "buy"){
            
            
            //获取商品数量
            let itemName = sentData["number"]!
            print("23434534===\(itemName)")
            if itemName == "0" {
//               MBProgressHUD.showError("请选择您要购买的商品", view: nil)
//                MBProgressHUD.drawErrorView(withText: "出错了", view: nil)
                MBProgressHUD.showNotice("请选择您要购买的商品", view: nil)
...
//                                    inDirectory: "HTML5")
//        let urlNow = URL(fileURLWithPath:path!)
//        let request = URLRequest(url:url)
        
        
        
//        webView?.load(URLRequest(url: urlNow))
        //2.加载网页
...
//        webView = WKWebView(frame:frame, configuration: theConfiguration)
//        //禁用页面在最顶端时下拉拖动效果
//        webView!.scrollView.bounces = false
        ...
        let path = Bundle.main.path(forResource: "listView", ofType: ".html")
...
//            webView?.scrollView.mj_header = MJRefreshNormalHeader(refreshingTarget: self, refreshingAction: #selector(self.headerRefresh))
//        webView?.scrollView.mj_footer = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: #selector(self.footerRefresh))
      
      
        
        
        // 添加进入条
        progressView = UIProgressView(frame: CGRect(x: 0, y: 64, width: view.frame.size.width, height: 2))
...
//            webView?.scrollView.mj_header.beginRefreshing()
            ...
            let footer = MJRefreshAutoGifFooter(refreshingTarget: self, refreshingAction: #selector(self.footerRefresh))
            //         设置普通状态的动画图片
...
//        self.webView!.evaluateJavaScript("loadData()",
//            completionHandler: nil)
//        
