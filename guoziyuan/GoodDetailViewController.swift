...
class GoodDetailViewController: BaseSubViewController,UITableViewDelegate,UITableViewDataSource,FSSegmentTitleViewDelegate,FSPageContentViewDelegate{
    var login:UIButton?
    var img:YLImageView?
    ...
    var goodsNumLabel:UILabel?
    static var goodsNum:NSInteger = 0
...
//        navigationItem.rightBarButtonItem = addNavBtn(cityNow!)
        ...
        //
        //        if (tableView == nil) {
        tableView = BaseTableView(frame: CGRect(x: 0, y: -20, width: view.bounds.width, height: SCREENHEIGHT-44), style: .plain)
        ...
        payView?.clickAction = {()-> Void in
            payView?.makeOrderButton?.isEnabled = false
...
//            payView?.makeOrderButton?.center
            
            let sss = Int((payView?.goodsNumberNow?.text)!)
//            print("eeeeeeee=====\(String(describing: sss!))")
            
            for index in 1...sss! {

                    
                    
                    self.showBuy(tiem:index-1)

  
            }
            
            let delay = DispatchTime.now() + .seconds(sss!)
            DispatchQueue.main.asyncAfter(deadline: delay) {
              payView?.makeOrderButton?.isEnabled = true
            }
        ...
        payView?.carClickAction = {()-> Void in
            let order = OrderViewController()
            order.title = "订单详情"
            self.isBuy = true
            order.isFromeShop = self.isfromShop
       ...
       goodsNumLabel = UILabel(frame: CGRect(x: (payView?.carMakeOrderButton?.center)!.x, y: (payView?.carMakeOrderButton?.frame.origin.y)!, width: 30, height: 15))
        payView?.addSubview(goodsNumLabel!)
        goodsNumLabel?.backgroundColor = UIColor.red
        goodsNumLabel?.textColor = UIColor.white
        goodsNumLabel?.textAlignment = .center
        goodsNumLabel?.font = UIFont.systemFont(ofSize: 10)
        goodsNumLabel?.layer.cornerRadius = 7
        goodsNumLabel?.clipsToBounds = true
        goodsNumLabel?.text = "\(String(GoodDetailViewController.goodsNum))+"
...
//            }
            ...
            payView?.goodsNumberNow?.text = String(describing: sss!+1)
//            print("1234543-----\(String(describing: sss!))")
            
        }
        
        
        
        payView?.deAction = {()-> Void in
            let sss = Int((payView?.goodsNumberNow?.text)!)
    ...
    func showBuy(tiem:Int){
        
        let delay = DispatchTime.now() + .seconds(tiem)
        DispatchQueue.main.asyncAfter(deadline: delay) {
            
            
            ShoppingCartTool.addToShoppingCart(withGoodsImage: UIImage(named: "heheda"), start:(CGPoint(x:SCREENWIDTH-50,y:SCREENHEIGHT-25)), end: CGPoint(x:SCREENWIDTH-145,y:SCREENHEIGHT-25), completion: {(_ finished: Bool) -> Void in
                print("动画结束了")
                GoodDetailViewController.goodsNum += 1
                //------- 颤抖吧 -------//
                let scaleAnimation = CABasicAnimation(keyPath: "transform.scale")
                scaleAnimation.fromValue = Float(1.0)
                scaleAnimation.toValue = Float(0.7)
                scaleAnimation.duration = 0.1
                scaleAnimation.repeatCount = 2
                // 颤抖两次
                scaleAnimation.autoreverses = true
                scaleAnimation.timingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseInEaseOut)
                self.goodsNumLabel?.layer.add(scaleAnimation, forKey: nil)
                self.goodsNumLabel?.text = "\(String(GoodDetailViewController.goodsNum))+"
...
//        navigationBar!.showCustomNavigationBar(false)
    }
