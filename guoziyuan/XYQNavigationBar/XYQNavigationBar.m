...
-(void)setDefalut{
    self.titleLable.textColor = blackColorOC;
...
-(void)setupBgColorAlpha:(CGFloat)alpha animation:(NSTimeInterval)duration compeleteBlock:(void (^)())compeleteBlock{
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        self.titleLable.textColor = [blackColorOC colorWithAlphaComponent:alpha];
        if (compeleteBlock) {
            compeleteBlock();
        }
    }];
...
-(UIImageView *)bgBroundView{
    if (!_bgBroundView) {
        _bgBroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bar"]];
    }
